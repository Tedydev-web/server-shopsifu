...
import { IAuthTokenResponse, IEncryptDataPayload } from '../interfaces/encryption.interface'
...
export class HelperEncryptionService implements IHelperEncryptionService {
	...
	constructor(
		...
		this.accessTokenSecret = this.configService.getOrThrow<string>('auth.accessToken.secret')
		this.refreshTokenSecret = this.configService.getOrThrow<string>('auth.refreshToken.secret')
		this.accessTokenExpire = this.configService.getOrThrow<string>('auth.accessToken.tokenExp')
		this.refreshTokenExpire = this.configService.getOrThrow<string>('auth.refreshToken.tokenExp')
	}

	public async createJwtTokens(payload: IAuthUser): Promise<IAuthTokenResponse> {
	...
	public async decrypt({ data, iv, tag, salt }: IEncryptDataPayload): Promise<string> {
		try {
			const key = await this.deriveKey(this.accessTokenSecret, Buffer.from(salt, 'hex'))
