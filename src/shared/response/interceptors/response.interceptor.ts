...
} from 'src/shared/doc/constants/doc.constant'
import { MessageService } from 'src/shared/message/services/message.service'
	...
	intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
		return next.handle().pipe(
			map(responseBody => {
				...
				const messageKey = this.reflector.get(DOC_RESPONSE_MESSAGE_META_KEY, context.getHandler())
				...
				if (
					...
					data.message = this.messageService.translateResponseMessage(data.message, lang)
				}
