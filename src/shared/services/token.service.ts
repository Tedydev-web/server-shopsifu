...
export class TokenService {
  ...
  signAccessToken(payload: AccessTokenPayloadCreate): string {
    return this.jwtService.sign(
      { ...payload, uuid: uuidv4() },
      {
        secret: envConfig.AUTH_ACCESS_TOKEN_SECRET,
        expiresIn: envConfig.AUTH_ACCESS_TOKEN_EXP,
        algorithm: 'HS256'
      }
    )
  }
  ...
  signRefreshToken(payload: RefreshTokenPayloadCreate): string {
    return this.jwtService.sign(
      { ...payload, uuid: uuidv4() },
      {
        secret: envConfig.AUTH_REFRESH_TOKEN_SECRET,
        expiresIn: envConfig.AUTH_REFRESH_TOKEN_EXP,
        algorithm: 'HS256'
      }
    )
  }
