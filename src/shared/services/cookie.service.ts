...
export class CookieService {
	...
	setAuthCookies(
		...
		const { name: accessTokenName, options: accessTokenOptions } =
			COOKIE_DEFINITIONS.accessToken
		res.cookie(accessTokenName, accessToken, accessTokenOptions)

		// Set refresh token cookie
		const { name: refreshTokenName, options: refreshTokenOptions } =
			COOKIE_DEFINITIONS.refreshToken
