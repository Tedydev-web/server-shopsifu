  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    return next.handle().pipe(
      map((data) => {
        ...
        if (data && typeof data === 'object' && data.data !== undefined && data.message !== undefined) {
          return data
        }

        // Nếu chỉ có message (như change password response)
        if (data && typeof data === 'object' && data.message !== undefined && data.data === undefined) {
          return data
        }

        // Nếu đã có data, metadata nhưng không có message thì wrap với message mặc định
        if (data && typeof data === 'object' && (data.data !== undefined || data.metadata !== undefined)) {
          return data
        } else {
