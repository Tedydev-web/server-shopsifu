...
export class HttpExceptionFilter extends BaseExceptionFilter {
  private readonly logger = new Logger(HttpExceptionFilter.name)
    ...
    const translatedResponse = this.translateErrorResponse(exceptionResponse, request)

    // Log error với context
    this.logger.error(`HTTP Exception: ${status} - ${JSON.stringify(translatedResponse)}`, exception.stack)
  ...
  private translateErrorResponse(exceptionResponse: any, request: any): any {
    ...
    if (typeof exceptionResponse === 'string') {
      const translatedMessage = i18n?.t(exceptionResponse) || exceptionResponse
      return {
        statusCode: 500,
    ...
    if (Array.isArray(exceptionResponse.message)) {
      const translatedMessages = exceptionResponse.message.map((error: any) => {
        if (error.message && typeof error.message === 'string') {
          const translatedMessage = i18n?.t(error.message) || error.message
          return {
            ...error,
            message: translatedMessage
          }
    ...
    if (exceptionResponse.message && typeof exceptionResponse.message === 'string') {
      const translatedMessage = i18n?.t(exceptionResponse.message) || exceptionResponse.message
      return {
        ...exceptionResponse,
        message: translatedMessage
      }
    ...
    if (exceptionResponse.error && typeof exceptionResponse.error === 'string') {
      const translatedError = i18n?.t(exceptionResponse.error) || exceptionResponse.error
      return {
        ...exceptionResponse,
