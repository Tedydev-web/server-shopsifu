...
export class CartRepo {
	...
	private async validateSKU({
		...
		const [cartItem, sku] = await Promise.all([
			this.databaseService.cartItem.findUnique({
				where: {
					userId_skuId: {
						userId,
						skuId
			...
			this.databaseService.sKU.findUnique({
	...
	async list2({
		...
		const totalItems$ = this.databaseService.$queryRaw<{ createdById: number }[]>`
      SELECT
        "Product"."createdById"
      FROM "CartItem"
    ...
    `
		const data$ = await this.databaseService.$queryRaw<CartItemDetailType[]>`
     SELECT
       "Product"."createdById",
       json_agg(
         jsonb_build_object(
   ...
   `
	...
	async create(userId: number, body: AddToCartBodyType): Promise<CartItemType> {
		...
		return this.databaseService.cartItem.upsert({
			where: {
				userId_skuId: {
					userId,
					skuId: body.skuId
	...
	async update({
		...
		return this.databaseService.cartItem
			.update({
				where: {
					id: cartItemId,
					userId
