...
export class ProductRepo {
	...
	async update({
		...
		const existingSKUs = await this.databaseService.sKU.findMany({
			where: {
				productId: id,
				deletedAt: null
			}
		...
		const [product] = await this.databaseService.$transaction([
			...
			this.databaseService.sKU.updateMany({
				where: {
					id: {
						in: skuIdsToDelete
					}
			...
			...skusToUpdate.map(sku =>
				this.databaseService.sKU.update({
					where: {
						id: sku.id as number
					},
					data: {
			...
			this.databaseService.sKU.createMany({
				data: skusToCreate
			})
		])
	...
	async delete(
		...
		const [product] = await Promise.all([
			...
			this.databaseService.productTranslation.updateMany({
				where: {
					productId: id,
					deletedAt: null
				},
