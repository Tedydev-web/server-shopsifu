...
export class OrderRepo {
	...
	async list(userId: number, query: GetOrderListQueryType): Promise<GetOrderListResType> {
		...
		// Đếm tổng số order
		const totalItem$ = this.databaseService.order.count({
			where
		})
		// Lấy list order
	...
	async create(
		...
		const [paymentId, orders] = await this.databaseService.$transaction(async tx => {
	...
	async cancel(userId: number, orderId: number): Promise<CancelOrderResType> {
		try {
			const order = await this.databaseService.order.findUniqueOrThrow({
				where: {
					id: orderId,
					userId,
					deletedAt: null
