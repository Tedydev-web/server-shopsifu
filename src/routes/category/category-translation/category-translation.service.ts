...
export class CategoryTranslationService {
	...
	async findById(id: number) {
		...
		return {
			data: category,
			message: this.i18n.t(
				'category.categoryTranslation.success.GET_DETAIL_SUCCESS'
			)
		}
	}
	...
	async create({
		...
		try {
			const categoryTranslation =
				await this.categoryTranslationRepo.create({
					createdById,
					data
				})
			return {
				...
				message: this.i18n.t(
					'category.categoryTranslation.success.CREATE_SUCCESS'
				)
			}
		} catch (error) {
	...
	async update({
		...
		try {
			...
			return {
				data: category,
				message: this.i18n.t(
					'category.categoryTranslation.success.UPDATE_SUCCESS'
				)
			}
		} catch (error) {
			if (isUniqueConstraintPrismaError(error)) {
	...
	async delete({ id, deletedById }: { id: number; deletedById: number }) {
		try {
			...
			return {
				message: this.i18n.t(
					'category.categoryTranslation.success.DELETE_SUCCESS'
				)
			}
		} catch (error) {
			if (isNotFoundPrismaError(error)) {
